...
const ViewReview = () => {
   ...
   const { data: ViewReview = [] } = useQuery({
      queryKey: ["viewReview"],
      queryFn: async () => {
   ...
   return (
      <div className="bg-reviewBg h-[650px]  bg-center bg-cover my-4 bg-fixed">
         <div className="h-[650px] bg-gradient-to-r py-40 px-4 from-[#040306CC] to-[#070315cc]">
            <Swiper
               slidesPerView={1}
               speed={1500}
               pagination={{
            ...
            >
               {ViewReview.map((view) => (
                  <SwiperSlide key={view._id}>
                     <div className="md:flex flex-col justify-center max-w-3xl p-6 shadow-md rounded-xl sm:px-12  text-gray-100 bg-[#1446574D] mx-auto">
                        <img src={view.image} alt="" className="w-32 h-32 mx-auto rounded-full bg-gray-500 aspect-square" />
                        <div className="space-y-4 text-center divide-y divide-gray-700">
                           <div className="my-2 space-y-1">
                              <h2 className="text-xl font-semibold sm:text-2xl">{view.reviewName}</h2>
                              <p className="px-5 text-xs sm:text-base text-gray-400">{view.review}</p>
                           </div>
